...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    /* ── Compact single-arc half-circle meter ── */
    function buildDelayMeter(delayVal, rowIdx) {
      ...
      // SVG 80x56: half-circle cx=40 cy=44 r=32
      // baseline y=44, arc top at y=12, full height fits in 56px
      const cx=40, cy=44, rad=32;
      const bgArc=`M ${cx-rad} ${cy} A ${rad} ${rad} 0 0 1 ${cx+rad} ${cy}`;
      let fillArc='';
      if (fillPct>0.5) {
      ...
      const ticks=[25,50,75,100].map(tp=>{
        const ta=Math.PI-(tp/100)*Math.PI;
        const x1=cx+(rad-4)*Math.cos(ta), y1=cy-(rad-4)*Math.sin(ta);
        const x2=cx+(rad+3)*Math.cos(ta), y2=cy-(rad+3)*Math.sin(ta);
        return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="rgba(255,255,255,.22)" stroke-width="1"/>`;
      }).join('');
      // Needle
      const needleAngle = Math.PI - (fillPct/100)*Math.PI;
      const nLen=rad-4;
      const ntx=cx+nLen*Math.cos(needleAngle), nty=cy-nLen*Math.sin(needleAngle);
      const bx=cx+(nLen-7)*Math.cos(needleAngle), by=cy-(nLen-7)*Math.sin(needleAngle);
      const px=Math.sin(needleAngle)*3, py=Math.cos(needleAngle)*3;
      const filtId=`gf-dl-${rowIdx}`;
      const valTxt = `${isDelayed?'+':''}${Math.round(delayVal)}d`;
      const lblTxt = isDelayed ? `△ +${Math.round(delayVal)}d` : isAhead ? `✔ ${Math.round(Math.abs(delayVal))}d` : `✔ On time`;
      const lblCol = isDelayed ? '#f43f5e' : isAhead ? '#10b981' : '#7c8db0';

      return `<div class="inline-meter-wrap${isDelayed?' m-delayed':''}">
        <svg viewBox="0 0 80 56" width="80" height="56" style="display:block">
          ...
          <!-- Dark bg panel -->
          <rect x="0" y="0" width="80" height="56" rx="7" fill="rgba(5,7,16,0.88)" stroke="${col}" stroke-opacity="0.28" stroke-width="1"/>
          <!-- Background track arc -->
          <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.09)" stroke-width="7" stroke-linecap="round"/>
          <!-- Tick marks -->
          ${ticks}
          <!-- Filled arc (single path, glow via filter) -->
          ${fillArc?`<path d="${fillArc}" fill="none" stroke="${col}" stroke-width="7" stroke-linecap="round" filter="url(#${filtId})"/>`:''}
          <!-- Needle stem -->
          <line x1="${cx}" y1="${cy}" x2="${bx.toFixed(2)}" y2="${by.toFixed(2)}" stroke="${col}" stroke-width="1.6" stroke-linecap="round"/>
          <!-- Needle tip arrowhead -->
          <polygon points="${ntx.toFixed(2)},${nty.toFixed(2)} ${(bx-px).toFixed(2)},${(by+py).toFixed(2)} ${(bx+px).toFixed(2)},${(by-py).toFixed(2)}" fill="${col}" style="filter:drop-shadow(0 0 4px ${col})"/>
          <!-- Pivot -->
          <circle cx="${cx}" cy="${cy}" r="3" fill="${col}" stroke="rgba(0,0,0,.7)" stroke-width="1.5"/>
          <!-- Value text above pivot -->
          <text x="${cx}" y="${cy-9}" text-anchor="middle" font-size="9.5" font-weight="800" fill="${col}" font-family="Inter,monospace,sans-serif">${valTxt}</text>
        </svg>
        <div style="font-size:.58rem;font-weight:700;color:${lblCol};letter-spacing:.3px;line-height:1;margin-top:2px">${lblTxt}</div>
      </div>`;
    }
